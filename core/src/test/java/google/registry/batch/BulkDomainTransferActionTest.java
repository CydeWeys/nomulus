  ...
  void beforeEach() throws Exception {
    ...
    activeDomain =
        persistDomainWithDependentResources(
            "active", "tld", now, now.minus(Duration.ofDays(1)), DateTimeUtils.END_INSTANT);
    ...
    pendingDeleteDomain =
        persistResource(
            persistDomainWithDependentResources(
                    "pendingdelete", "tld", now, now.minus(Duration.ofDays(1)), plusDays(now, 30))
                .asBuilder()
                .setStatusValues(ImmutableSet.of(StatusValue.PENDING_DELETE))
                .build());
    deletedDomain = persistDeletedDomain("deleted.tld", toDateTime(now.minus(Duration.ofDays(30))));
  }
